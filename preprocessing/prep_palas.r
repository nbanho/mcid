...
#' However, we encounter CO2 levels even below 400, so maybe near the schools the
#' CO2 levels were actually lower, or the measurements are inaccurate below 400.

df_full %>% filter(week == 6) %>% select(school, class, date, time, co2ppm) %>% ggplot(aes(x = time, color = school, y = co2ppm)) + facet_wrap(~date) + geom_point()

#' The measurements during weeks without school are not consistent between schools but vary around 400 to 420, so in the range of outdoor levels.
#' We therefore set a minimum of 400 and set all values below 400 to 400.

co2ppm_out <- 400

df_filt$co2ppm <- ifelse(df_filt$co2ppm < co2ppm_out, co2ppm_out, df_filt$co2ppm)
df_full$co2ppm <- ifelse(df_full$co2ppm < co2ppm_out, co2ppm_out, df_full$co2ppm)
...
#' at the top the values are continuous --> probably no outliers --> keep
...
#' some almost vertical lines, i.e. sudden changes --> remove
...
hist(df_filt$co2ppm[df_filt$co2ppm<500])
hist(df_filt$co2ppm)

co2ppm_out <- round(min(df_filt$co2ppm, na.rm = T))
print(co2ppm_out)
...
df_filt <- df_filt %>%
  mutate(f = ((co2ppm + 1) - co2ppm_out) / co2ppm_ex, # add 1 in case co2ppm is exactly equal to co2ppm_out 
         f0 = f * (n - 1) / n,
         rav = p * f0,
         Q = 3600 * G / (((co2ppm+1) - co2ppm_out) / 1e3), # add 1 in case co2ppm is exactly equal to co2ppm_out
         aer = Q * n / V,
...
df_filt <- df_filt %>%
  select(school, class, date, week, weekday, time, intervention, maskmandate, airfilter, 
