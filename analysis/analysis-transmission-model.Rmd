...
df <- read_csv("../data-clean/redcap.csv") %>%
  filter(!no_school) %>%
  mutate(contagious = n_class - (cum_recovered_from_confirmed + cum_recovered_from_symptomatic + cum_recovered_from_isolated),
         new_cases = new_confirmed_by_onset + new_symptomatic + new_isolated, 
         cum_cases = cum_confirmed_by_onset + cum_symptomatic + cum_isolated) %>%
...
df_school <- df %>%
  group_by(school, day, date, weekend) %>%
  summarize(across(c(new_cases, cum_cases, contagious, population, absences), sum)) %>%
  ungroup() %>%
  mutate(prop_absences = absences / population,
         prop_contagious = contagious / population) %>%
  left_join(r_est)
```

```{r}
df_comp <- df_school %>%
  select(school, date, day, cum_cases, contagious) %>%
  melt(c("school", "date", "day")) %>%
  mutate(variable = ifelse(grepl("cases", variable), "Cases", "Contagious")) %>%
  mutate(type = "Observed")

df_comp_new <- df_school %>%
  select(school, date, day, new_cases) %>%
  melt(c("school", "date", "day")) %>%
...
```{r}
df_school %>%
  select(school, day, cum_cases, contagious, population) %>%
  melt(c("school", "day")) %>%
...
sDF <- list(
  ...
  new_cases = df_school %>%
    ...
    as.matrix(),
  prop_contagious = df_school %>%
    select(school, day, prop_contagious) %>%
    spread(school, prop_contagious) %>%
    select(-day, `School 1`, `School 2`) %>%
    as.matrix(),
...
```{r}
tm <- cmdstan_model("../models/tm_school.stan", cpp_options = list(stan_threads = T))
...
new_inc_pl <- ggplot(mapping = aes(x = day)) +
  ...
  geom_bar(data = df_comp_new, mapping = aes(y = -value, fill = variable, linetype = type), stat = "identity", size = 1) +
