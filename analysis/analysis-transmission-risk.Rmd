...
infectious_sub_pl <- infectious_sub %>%
  ...
  geom_boxplot() +
  scale_color_manual(values = intervention_col) +
  labs(y = "Expected number of infectious students", title = "b") +
  theme_bw2() +
...
comb_input_pl <- arrangeGrob(rebAirFrac_pl, infectious_sub_pl, ncontacts_pl, widths = c(4.4, 4.4, 7.2), ncol = 3)
ggsave(plot = comb_input_pl, filename = "../results/supp-transrisk-input.pdf", width = 16 / cm(1), height = 6 / cm(1))
```
