...
Estimate transmission risk for 8 hours school day.
...
q_red <- data.frame(
  ...
  masks = c(0.81, 0.66, 0.90),
  aircleaner = c(0.50, 0.13, 0.73)
)
...
transRisk_pl <- ggplot(mapping = aes(x = intervention)) +
  ...
  geom_hline(data = transRisk %>% filter(intervention == "No intervention"), aes(yintercept = mean),
             linetype = "dashed") +
  geom_text(data = transRisk %>% filter(intervention == "No intervention"), aes(label = "No intervention", y = mean + 2, 1),
            size = 8 / cm(1)) +
...
transRisk %>%
  ...
  select(intervention, activity, mean, lower, upper)
