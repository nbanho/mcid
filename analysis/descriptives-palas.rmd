...
df_merge_mean <- df_merge %>%
  ...
  summarize(across(c(week, day, intervention), ~ .x[1]),
            across(c(co2ppm, rh, cn1cm, pmtotmugm, vocmgm, rav), ~ mean(.x, na.rm = T)),
            n_in_class = n_class[1] - n_tot_absent[1]) %>%
  ungroup() %>%
...
```{r}
htmlreg(list(co2_lm, rh_lm, cn1cm_lm, pmtotmugm_lm, vocmgm_lm, rav_lm),
        ...
        PM total: particle matter mass concentration total (mug/m3), 
