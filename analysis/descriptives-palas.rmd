...
boxplots_main <- df_daily %>%
  ...
  theme_bw2() +
  theme(legend.position = "bottom", axis.title = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank())
...
effects_env <- draws_filt %>%
  ...
  stat_pointinterval(position = position_dodge(width = .5), fill = "white", shape = 21, .width = c(.5, .8, .95)) +
...
effects_env_main <- draws_filt %>%
  ...
  stat_pointinterval(fill = "white", shape = 21, .width = c(.5, .8, .95)) +
  ...
  geom_vline(aes(xintercept = 0), linetype = "dotted") +
  theme_bw2() +
  theme(axis.title.y = element_blank(), legend.position = "bottom", axis.ticks.y = element_blank(), axis.text.y = element_blank())
