...
cum_inc_pl <- ggplot(mapping = aes(x = day)) +
  ...
  scale_x_continuous(expand = c(0,0), breaks = c(-11, seq(1, 51, 10))) +
  scale_y_continuous(expand = c(0,0)) +
  labs(y = "Cumulative incidence", color = "", fill = "", linetype = "", title = "b") +
  theme_bw2() +
  theme(axis.title.x = element_blank(), legend.position = "bottom",
        panel.spacing = unit(1, "lines"), plot.title = element_text(hjust = 0))
...
new_inc_obs <- meta_dat %>%
  ...
  median_qi(.width = .95) %>%
  ungroup() %>%
  mutate(variable = "Observed (simulated)") %>%
  rename(value = new_cases) %>%
  select(school_class, day, variable, value, .lower, .upper)
  
# coverage
coverage_df <- new_inc_mean %>%
  filter(day >= 1) %>%
  mutate(variable = "Estimated") %>%
  ...
  summarise(across(c(value, .lower, .upper), sum)) %>%
  ungroup()

coverage_df_pl <- coverage_df %>%
  ggplot(aes(y = intervention, color = variable)) +
  facet_wrap(~ school_class) + 
  geom_linerange(mapping = aes(xmin = .lower, xmax = .upper), position = position_dodge(width = .25)) +
  geom_point(mapping = aes(x = value, shape = variable), position = position_dodge(width = .25)) +
  scale_color_manual(values = c("blue", "black")) +
  labs(x = "Estimated and observed (simulated) number of cases") +
  theme_bw2() +
