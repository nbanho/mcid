...
total_freq <- sim_df  %>%
  ...
  summarise(n = mean(n)) %>%
  ungroup() %>%
  mutate(intervention = factor(intervention, levels = rev(levels(intervention))),
         class = factor(class, levels = c("Study (A)", "Study (B)", "Study", "Control"))) 

total_freq %>% 
  group_by(intervention) %>% 
  summarise(n = round(sum(n), 1)) %>%
  ungroup()

total_freq_pl <- total_freq %>%
  ggplot(aes(x = class, y = n, fill = intervention)) +
  geom_bar(position = "stack", stat = "identity", width = .5, alpha = .75) +
...
cases_time_pl <- sim_modeling_df %>%
  ...
  labs(y = "Number of new cases", title = "b") +
  theme_bw2() +
...
comp_sim_pl <- sim_modeling_df %>%
  ...
  geom_step(aes(color = "Expected cases after probabilistic simulation")) +
  facet_wrap(~ school_class, ncol = 1) +
