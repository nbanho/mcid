...
cases_time_pl <- sim_modeling_df %>%
  ...
  facet_wrap(~ school_class, ncol = 1, strip.position = "left", scales = "free_y") +
  scale_fill_manual(values = c(intervention_col, cbPalette[3:4], "white")) +
  scale_x_date(expand = c(0,0)) +
  scale_y_continuous(expand = expansion(mult = c(0, .05)), breaks = seq(0, 25, 5)) +
...
cases_time_pl
save_plot(cases_time_pl, pdf_file = "../results/redcap-descriptives.pdf", w = 16, h = 16)
```
...
mol_samples_pl <- cases_mol_sum %>%
  ...
  labs(y = "Proportion of positive samples per week (%)", title = "a") +
  theme_bw2() +
