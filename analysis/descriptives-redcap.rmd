...
cases_air <- tibble(
  ...
  `Saliva` = c(1, 2, 0, 2, 6, 1, 0, 12,
               1, 0, 2, 0, 1, 3, 0, 7),
  `Air samples` = c(0, 0, 0, 2, 0, 0, 2, 4, 
                    1, 3, 0, 0, 1, 0, 0, 5),
  `\\hphantom{00}Copies/l (Vivas)` = c("0", "0", "0", "2.46", "0", "0", "0.36", "",
                           "1.27", "2.68", "0", "0", "2.07", "0", "0", "") ,
  `\\hphantom{00}Copies/l (Coriolis)` = c("0", "0", "0", "0", "0", "0", "0.12", "",
                              "0", "0.16", "0", "0", "0", "0", "0", ""),
  `Air samples, filter 1` = c(rep("", 7), 2, 
                   rep("", 7), 4),
  `Air samples, filter 2` = c(rep("", 7), 0, 
                   rep("", 7), 3),
...
cases_mol_sum <- cases_mol %>%
  select(matches(c("school", "week", "Saliva", "Air samples", "Vivas"))) %>%
  select(-matches("filter")) %>%
  set_names(c("school", "study_week", "Saliva", "Air", "vivas")) %>%
  filter(study_week < 8) %>%
  mutate(vivas = as.numeric(vivas)) %>%
  mutate(intervention = ifelse(school == "School 1", ifelse(study_week <= 2, "Mask mandate",
                               ...
                               ifelse(study_week <= 4, "Mask mandate", 
                                      ifelse(study_week <= 5, "No intervention",
                                             "Air cleaner"))),
         intervention = factor(intervention, levels = c("Mask mandate", "No intervention", "Air cleaner"))) %>%
  group_by(school, intervention) %>%
  summarize(across(c(Saliva, Air, vivas), mean, na.rm = T)) %>%
  ungroup() 
...
mol_samples_pl <- cases_mol_sum %>%
  ...
  mutate(variable = factor(variable, levels = c("Air", "Saliva"))) %>%
  ggplot(aes(x = as.integer(intervention), y = value, color = school, linetype = variable)) +
  geom_point() +
  geom_line() +
  ...
  labs(y = "Positive samples per week", title = "a") +
  theme_bw2() +
...
mol_copies_pl <- cases_mol_sum %>%
  select(-Air, -Saliva) %>%
  ggplot(aes(x = as.integer(intervention), y = vivas, color = school)) +
  geom_point() +
  geom_line() +
  scale_x_continuous(breaks = c(1,2,3), labels = c("Mask mandate", "No intervention", "Air cleaner")) +
  scale_y_continuous(limits = c(0, 3)) +
  scale_color_manual(values = RColorBrewer::brewer.pal(3, "Set2")[c(2,3)]) +
  labs(y = "Copies/l per week", title = "b") +
  theme_bw2() +
