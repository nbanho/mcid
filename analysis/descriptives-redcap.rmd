...
ggsave(plot = comb_pl, filename = "../results/redcap-descriptives.pdf", width = 16 / cm(1), height = 16 / cm(1))
```

### Cases by intervention

```{r}
prop_suspected_pl <- df_new %>%
  group_by(school, intervention, variable) %>%
  summarize(value = sum(value, na.rm = T)) %>%
  ungroup() %>%
  ggplot(aes(x = intervention, y = value, fill = variable)) +
